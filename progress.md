...
| Question | Answer |
|----------|--------|
| Where am I? | Phase 6 complete - tables tested, archiving |
| Where am I going? | Archive goal-7 |
| What's the goal? | Rich text support for Notes.app |
| What have I learned? | Native styles are a platform limitation, tables work via HTML |
| What have I done? | Markdown-to-HTML conversion with table support |

---

## Session 4: Table Support (2026-01-20)

### Table Testing Results
- Tables stored as **embedded attachments** (`com.apple.notes.table`)
- Plain text read shows `￼` placeholder character
- HTML read returns full `<object><table>...</table></object>`
- Tables CAN be created via HTML in AppleScript body
- Round-trip works if HTML is preserved

### Markdown Table Support Added
- Added table parsing to `MarkdownConverter.swift`
- Syntax: `| col | col |` with `|---|---|` separator
- Converts to Notes.app native table format
- Fixed extra `<br>` tags issue (removed newlines from table HTML)
- Refactored `AppleScript.processBody()` to use `MarkdownConverter` class

### Files Modified
- `Sources/NotesLib/Notes/MarkdownConverter.swift` - Added `processTables()`, `parseTable()`, `parseTableRow()`
- `Sources/NotesLib/Notes/AppleScript.swift` - Use `MarkdownConverter` in `processBody()`
- `findings.md` - Added Table Support section
- `task_plan.md` - Updated Phase 3 and 6
